...
def batch_add_interactions_to_video_interactions_table(db, video_data_list):
    db.executemany(
        'INSERT INTO video_interactions (video_id, view_count, like_count, comment_count, latest) VALUES (?, ?, ?, ?, ?)', video_data_list
    )


def update_most_recent_interactions_in_video_interactions_table(db, video_pk_list):
    db.executemany(
        'UPDATE video_interactions SET latest = 0 WHERE video_id = ? AND latest = 1', video_pk_list
    )
...
def get_video_from_videos_table_by_channel_id(db, channel_id):
